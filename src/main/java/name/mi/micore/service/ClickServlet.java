...
import name.mi.buyer.brokersweb.xml.BrokersXmlClickPost;
import name.mi.buyer.brokersweb.xml.BrokersXmlClickResponse;
...
import name.mi.micore.util.BuyerAccountConfigComparator;
...
@WebServlet(asyncSupported = false, name = "ClickServlet", urlPatterns = {"/Click"})
public class ClickServlet extends HttpServlet {
    ...
    private ReplyWithResult processRequest(HttpServletRequest iRequest)
    {
        ...
        try
        {
            ...
            if (vZipCodeString == null || vZipCodeString.isEmpty()|| vZipCodeString.equals("00000"))
            {
                Arrival vArrival = ArrivalDAO.getArrivalByID(LOGGER, vConnection, vArrivalID);
                vZipCodeString = IPMapDAO.getZipCodeByIP(LOGGER, vConnection, vArrival.getIPAddress());
            }
            ...
            // choose Buyer_Account, may be changed logically according to the buyer information or request
            BuyerAccountConfig vBuyerAccountConfig = chooseBuyerAccountConfig(LOGGER, vConnection, vClickImpressionRequest);
            BuyerAccount vBuyerAccount = BuyerAccountDAO.getBuyerAccountByID(LOGGER, vConnection, vBuyerAccountConfig.getBuyerAccountID());

            // get Click_Ads according to vBuyerAccount
            List<ClickAd> vClickAdList = new ArrayList<ClickAd>();

            // Keep it, DO NOT DELETE
            if (vBuyerAccount.getAccountName().equals("WebJuiceForAutoInsurance"))
            {
                vClickAdList.addAll(getWebJuiceClickAds(vConnection, vClickImpressionRequest, vSiteName));
            }
            if (vBuyerAccount.getAccountName().equals("SureHitsForAutoInsurance"))
            {
                // limit SureHits click out to 25 per day
                SimpleDateFormat vSimpleDateFormat =  new SimpleDateFormat("yyyy-MM-dd");
                if (!vSimpleDateFormat.format(vBuyerAccountConfig.getUpdatedTS()).equals(vSimpleDateFormat.format(new Date())))
                {
                    BuyerAccountConfigDAO.resetBuyerAccountConfigCountByID(LOGGER, vConnection, vBuyerAccountConfig.getID());
                    vClickAdList.addAll(getSureHitsClickAds(vConnection, vClickImpressionRequest, vLeadRequest, vSiteName));
                }
                else if (vBuyerAccountConfig.getCount() < vBuyerAccountConfig.getLimit())
                {
                    vClickAdList.addAll(getSureHitsClickAds(vConnection, vClickImpressionRequest, vLeadRequest, vSiteName));
                }
                else
                {
                    vClickAdList.addAll(getBrokersXmlClickAds(vConnection, vClickImpressionRequest, vLeadRequest, vSiteName));
                }
            }
            if (vBuyerAccount.getAccountName().equals("BrokersWebForAutoInsurance"))
            {
                vClickAdList.addAll(getBrokersXmlClickAds(vConnection, vClickImpressionRequest, vLeadRequest, vSiteName));
            }
...
//    }
    ...
    private List<ClickAd> getBrokersXmlClickAds(Connection iConnection, ClickImpressionRequest iClickImpressionRequest, LeadRequest iLeadRequest, String iSiteName) throws Exception
    {
        long vBrokersXmlAccountID = 12;
        ...
        try
        {
            ...
            String vUrl = BrokersXmlClickPost.getClickUrl();
            List<NameValuePair> vNameValuePairList = BrokersXmlClickPost.getClickParameterList
                    (
                            LOGGER,
                            iConnection,
                            vClickImpression,
            ...
            return BrokersXmlClickResponse.getAds(vResponseData, vClickImpression.getID(), vClickImpression.getToken());
        }
        catch (Exception e)
        {
            LOGGER.error("getBrokersXmlClickAds error: " + e);
            return null;
        }
    }
