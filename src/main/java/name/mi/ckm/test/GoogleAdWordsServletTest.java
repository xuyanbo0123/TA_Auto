...
public class GoogleAdWordsServletTest {
    ...
    private static void testGoogleAdWords()
        throws Exception
    {
        ArrayList<NameValuePair>
            vList = new ArrayList<NameValuePair>();

        vList.add(new BasicNameValuePair("operator", "testOperator"));
        //vList.add(new BasicNameValuePair("task", "unlockEmergency"));
        //vList.add(new BasicNameValuePair("task", "downloadGoogleToLocal"));
        vList.add(new BasicNameValuePair("task", "uploadLocalToGoogle"));

        System.err.println("before response at " + new Date());

        String
            vResponse = postParameters(
            "http://localhost:8080/ta-auto/GoogleAdWords",
            20 * 1000,
